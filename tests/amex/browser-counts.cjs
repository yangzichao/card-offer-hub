...
async function run() {
    ...
    try {
        ...
        // One unattended run: two offers, one request each, initially 1s apart, nothing overlapping.
        ...
        assert.ok(enrollmentRequests[1].startedAt - enrollmentRequests[0].startedAt >= 1000);
        ...
        console.log('PASS: seven-card scan, cross-card counts, reordered priority re-targeting shared offers, one-card-per-offer allocation, serial 1s enrollment, and no repeat run.');
    } finally {
        await browser.close();
    }
}
