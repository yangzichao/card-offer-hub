...
async function run() {
    ...
    try {
        ...
        console.log('PASS: real Chromium UI, manual-only startup, whitelist-only requests, complete lists, initial 1s spacing, enrollment/filter state, SPA remount, cancellation, 429 cooldown, and narrow viewport.');
    } finally {
        await browser.close();
    }
}
