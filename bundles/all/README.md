...
单独版与合并版使用同一套界面和 **Choose scope → Scan offers → Review & add** 三步流程。搜索只过滤显示，Add all offers 的范围不随搜索改变；跨银行搜索仅在合并版提供。详见[统一操作流程](../../docs/unified-workflow.md)。首次改用合并版，需要在每家银行手动扫描一次；仅安装合并版不会自动获取其他银行数据。
