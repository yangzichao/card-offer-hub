...
- Amex 的 functions 网关限流时回 429 但不带 CORS 头，浏览器只报 "Failed to fetch"。所以发往这个网关的网络错误也按 429 处理：停止本轮、冷却、间隔翻倍，状态栏显示 "Amex stopped answering…"。那个 Offer 记为未确认，要扫描核实。冷却结束后手动点 Add all，从没发过的 Offer 接着加。超时和同源请求的网络错误不算限流。
