...
2. 首次使用点击 **Load cards & offers**，读取全部卡片和优惠，然后勾选要处理的卡片。新卡默认不选，选择会保存。
...
- 429 按 `Retry-After` 冷却；HTTP 错误、未知响应、超时均停止，使用 Refresh & add offers 核验后再继续。没有自动重试。
...
- 带 schema 版本的快照保存卡片、勾选、列表和登记结果。恢复后先显示缓存；继续添加会核对当前登录的卡片，若已选卡片不再属于当前登录则停止。刷新保留仍有效的选择，新卡不自动勾选；任一已选卡片消失时，先停止并要求检查选择。存储错误会显示并阻止新请求。
...
保存失败会显示错误；未知或损坏的快照保留原数据，不静默覆盖。快照只保存展示字段和选择，Cookie、登录 token、请求头、原始响应和地理位置不写入快照。正在添加时刷新，未确认的项目保留为未确认，后续必须选择 Refresh & add offers 先核验，再处理仍可用的项目。直接使用缓存可能遇到已在网站添加或已失效的优惠；收到错误或不明确的响应就停止，不暗中刷新或重试。详见[持久化说明](../../../docs/local-persistence.md)和[继续操作与全量刷新](../../../docs/citi-refresh-continuation.md)。
