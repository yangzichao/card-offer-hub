...
- 单条登记返回 HTTP 5xx 或 HTTP 200 未明确确认时，保存为未确认并跳过，继续其他可用优惠，不重试该条。连续三条登记返回 5xx 且仍有剩余项时暂停并保存未提交队列，防止持续故障耗尽队列。进度和具体错误显示在操作按钮下方。
- 429 按 `Retry-After` 冷却；读取失败、HTTP 4xx、网络故障、非 JSON 响应和超时仍停止。服务器错误不会误提示重新登录。
