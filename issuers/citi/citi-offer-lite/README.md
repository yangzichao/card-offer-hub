...
3. 之后只需在两个操作中选一个：**Add saved offers** 直接尝试缓存中仍标为 `AVAILABLE` 的优惠，正常缓存不重新扫描优惠；会话变化或上次失败时，同一次点击先核验当前状态再添加；**Refresh & add offers** 刷新所有当前卡片及优惠后，自动添加已选卡上的可用优惠，无需再点添加。两条路径都会先核对所选卡片属于当前登录。缓存数量为零时仍可刷新并添加新优惠。
...
- 带 schema 版本的快照保存卡片、勾选、列表和登记结果。恢复后先显示缓存；继续添加会核对当前登录的卡片，如果旧标识无法核对则保留缓存并停止；完整刷新会重新建立当前卡片列表。刷新保留仍有效的选择，新卡默认勾选；完整刷新不会被已消失的旧卡片阻止，全部读取成功后才替换缓存。存储错误会显示并阻止新请求。
