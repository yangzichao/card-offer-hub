...
所有银行和安装方式共用 `shared/ui/workflow-layout.js`，统一为三个步骤：

1. **Choose scope**：选择卡片，或确认当前账户／profile。
2. **Scan offers**：手动更新扫描结果；**Stop** 停止后续请求。
3. **Review & add**：搜索已保存结果，点击 **Add all offers** 添加所选范围内全部可添加项。

搜索只影响列表显示，不改变添加范围。主按钮显示全部可添加数量，并说明未选范围、需要扫描、无可添加项、存储错误、冷却或只读等状态。运行中的按钮保持原名并禁用，另行显示任务进度。状态文字统一为 Available、Added、Needs review 和 Skipped。原始银行状态与成功判定保留。
...
- Chase 仍为只读，Add all offers 禁用并明确解释；可扫描和搜索。
- Citi 的扫描入口为 **Refresh all cards & offers**，一次更新全部卡片和优惠，不受添加勾选范围限制。恢复保存结果后可直接点击添加；同一次点击先核验当前卡片和优惠状态，无需分步重新检测或扫描。未确认项目仍需先刷新。详见[继续操作与全量刷新](citi-refresh-continuation.md)。
- 其他银行恢复缓存后的扫描要求按既有协议保留，所有银行禁止自动扫描、自动添加、自动重试或保活。
