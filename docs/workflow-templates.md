...
Citi 与 Chase 提供使用缓存添加和刷新后直接添加两个入口，均先核对当前登录卡片；刷新入口读取所有卡后只添加已选卡，新卡默认勾选，保留主动取消的选择。Chase 的缓存入口还会读取所选卡的当前状态与点击参数，只处理原有缓存优惠，跳过仍未确认的记录；初次加载合并卡片发现与优惠读取，不添加。Chase 保留对网页请求的被动观察，不在页面加载时自动发请求。
...
上下文为 `selectedCardIds` 与完整的 `priorityCardIds`。每组优惠选择优先级最高的可用卡；卡片独有的优惠保留。已添加或未确认的组选中卡记录会阻止该组改投另一张卡。资格、完整扫描与可添加性由 Amex 适配器转换；登记返回「已在另一张卡」（`PZN4107`）的记录也按已添加处理，分组规则继续使用当前银行的 `groupKey`；本次没有改变分组算法。
