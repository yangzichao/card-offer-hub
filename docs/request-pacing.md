...
当前实现已升级为[按银行自动调速与本地学习](adaptive-pacing.md)。下文保留固定 500ms 阶段的维护记录，不代表当前默认速度。

- 本次发布将 Amex 和 Citi 正常请求的等待间隔从 15 秒改为 500 毫秒，添加及扫描共用该间隔。其余四家银行脚本也已同步调整，并在用户随后授权的完整发布中一并提交。
...
用户随后明确要求推送全部本地项目改动。本次追加发布 BankAmeriDeals、Chase、US Bank、Wells Fargo 1.0.0，以及全部配套源码、合成测试、文档、安装目录和构建产物。Chase 当前仅提供扫描，不支持添加。账户抓包、凭证与忽略的临时文件不进入提交；session 保活未实现。
