...
- 地址优先避免漏匹配：`matches` 覆盖银行主域名及子域名，`adapterMatches` 列出有已知 API 合约的完整 HTTPS 域名并使用 `/*`。其他匹配域名显示优惠入口和缓存搜索。两层路由及 `offersUrl` 必须同步；参见 `docs/website-matching.md`。
...
- **限速串行。** 同一时刻只有一个请求在飞，间隔从上一个响应完成开始算。429 要按 `Retry-After` 进入冷却，且已有的更长冷却不能被更短的覆盖。发卡行把 429 藏成网络错误时也按限流处理：Amex functions 网关的 429 不带 CORS 头，页面只看到 `Failed to fetch`，所以 Amex 发往该网关的网络错误按 429 冷却并降速。
- **不自动重试。** 不要盲目重发失败或未确认的优惠。Citi 的单条 HTTP 200 登记结果未确认或登记返回 HTTP 5xx 时保存并跳过，继续其他可用优惠；连续三条登记返回 5xx 且仍有剩余项时暂停，保留未提交队列。读取失败、HTTP 4xx、网络故障、登录异常、限流和存储故障仍停止并要求显式核验。Citi 下次手动点击添加可在同一次操作内先核验缓存状态，再继续尚未提交的优惠；历史失败标记不能永久禁用按钮。Amex 的单条 HTTP 200 登记回答（明确拒绝、`PZN4107` 已在另一张卡、未确认）记录后继续下一个优惠，每个优惠只发一次，被拒的优惠要等下一次手动点击才换卡；连续三条未确认且仍有剩余项时暂停。Amex 的 HTTP 错误、网络故障、超时、非 JSON、429 和存储故障仍停止本轮。
