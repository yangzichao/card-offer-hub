...
- **不自动重试。** 不要盲目重发失败或未确认的优惠。Citi 的单条 HTTP 200 登记结果未确认时保存并跳过，继续其他可用优惠；HTTP 错误、网络故障、登录异常、限流和存储故障仍停止并要求显式核验。Citi 下次手动点击添加可在同一次操作内先核验缓存状态，再继续尚未提交的优惠；历史失败标记不能永久禁用按钮。
